...
public class Calculo<V> {
    ...
    public Calculo(Graph<Usuario, Relacion> g) {
        ...
        for (Edge<Relacion> e : datos.edges())
            dijkstra.insertEdge(mVertex.get(datos.endVertices(e)[0].getElement().getCodigo()),
                    mVertex.get(datos.endVertices(e)[1].getElement().getCodigo()),
     ...
     */
    public List<Usuario> antiguedad(String src, String target) {
        PositionalList<Vertex<Usuario>> aux = new LinkedPositionalList<>();
        aux=GraphAlgorithms.shortestPathList(dijkstra, mVertex.get(src), mVertex.get(target));
        List<Usuario> answer = new ArrayList<Usuario>();
        for(Vertex<Usuario> l : aux)
            answer.add(0,l.getElement());
        return answer;
    }
