...
public abstract class AbstractTree<E> implements Tree<E> {
	...
	public List<Position<E>> searchAll(E e) {
		Iterable<Position<E>> i = positions();
		List<Position<E>> l = new ArrayList<>();
	...
	public boolean duplicate() {
		for (Position<E> n : positions())
			if (searchAll(n.getElement()).size() > 1)
				return true;
		return false;
	}
	...
	/**
	 * Retorna un Map donde la clave es la profundidad y el valor una lista con las
	 * posiciones de todos los nodos que tienen esa profundidad.
	 *
	 * @return Map clave: profundidad, valor: lista de nodos con esa profundidad
	 * 
	 */
	public Map<Integer, List<E>> mapDepth() {
		Map<Integer, List<E>> md = new ChainHashMap<Integer, List<E>>();
		List<E> l;
		Integer d;
		for (Position<E> p : positions()) {
			d = depth(p);
